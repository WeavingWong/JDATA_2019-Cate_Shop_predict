...
def get_user_features():
    ...
    ## (1)统计个人信息
    Path = '../data/features/features_a_test/userInfo.csv'
    if os.path.exists(Path):
        ...
        userInfo = pd.concat([userInfo[['user_id', 'user_reg_tm']], df_user_reg_time, df_age, df_sex, df_city_level, df_province], axis=1)
        del df_user_reg_time, df_age, df_sex, df_province, df_city_level
        userInfo['reg_tm_last_day'] = userInfo['user_reg_tm'].apply(lambda x: 1 if x == end_time else 0)
        userInfo['reg_tm_7_day'] = userInfo['user_reg_tm'].apply(lambda x: 1 if (x <= end_time) & (x >= end_time_7) else 0)
        userInfo.drop('user_reg_tm', axis=1, inplace=True)
        # userInfo.to_csv('../data/features/features_a_test/userInfo.csv', index=False, encoding='utf-8')
    user_features = pd.merge(user_features, userInfo, on='user_id', how='left').fillna(0)
    print('userInfo')

    ## (2)统计用户的购买频次，用户的平均购买时间间隔, 最后一次购买距离考察周时间
    Path = '../data/features/features_a_test/user_buy_info.csv'
    if os.path.exists(Path):
        ...
        usertmp['u_b2_last_day_start'] = (pd.to_datetime(label_time_start) - pd.to_datetime(usertmp['action_time'])).dt.days
        ...
        # userSub.to_csv('../data/features/features_a_test/user_buy_info.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/u_n_counts.csv'
    if os.path.exists(Path):
        ...
        userSub.rename(
            columns={'cate': 'u_cate_counts', 'shop_id': 'u_shop_counts', 'action_time': 'u_active_days_in_all'}, inplace=True)
        ...
        # u_n_counts.to_csv('../data/features/features_a_test/u_n_counts.csv', index=False, encoding='utf-8')
    user_features = pd.merge(user_features, u_n_counts, on='user_id', how='left').fillna(0)
    print('u_n_counts')
    user_features.to_csv('../data/features/features_a_test/user_features.csv', index=False, encoding='utf-8')
    print('user_features!!!!')
...
def get_ucs_features():
    ...
    # (1)统计点击购买转化率，加入购物车购买转化率，被收藏次数购买转化率, 购买评论转化率
    Path = '../data/features/features_a_test/ucs_b_rate.csv'
    if os.path.exists(Path):
        ...
        # ucs_b_rate.to_csv('../data/features/features_a_test/ucs_b_rate.csv', index=False, encoding='utf-8')
    ucs_features = pd.merge(ucs_features, ucs_b_rate, on=['user_id', 'cate', 'shop_id'], how='left').fillna(0)
    print('ucs_b_rate')

    # (2)ucs_b_count_in_n(n=1/3/5/7; 用户对店铺—品类对在考察日前n天的行为总量计数，考察周取2.21-2.27)
    Path = '../data/features/features_a_test/ucs_b_count_in_n.csv'
    if os.path.exists(Path):
        ucs_b_count_in_n = pd.read_csv(Path)
    else:
        userSub = userAll[(userAll['action_time'] > end_time_1) & (userAll['action_time'] <= end_time)]
        userSub = userSub[['user_id', 'cate', 'shop_id', 'type_1', 'type_2', 'type_3', 'type_4', 'type_5']]
        userSub = userSub.groupby(['user_id', 'cate', 'shop_id'], as_index=False).sum()
        userSub['ucs_b_count_in_1'] = userSub['type_1'] + userSub['type_2'] + userSub['type_3'] + userSub['type_4'] + userSub['type_5']
        userSub.drop(['type_1', 'type_2', 'type_3', 'type_4', 'type_5'], axis=1, inplace=True)
        ...
        ucs_b_count_in_n = pd.merge(ucs_b_count_in_7, ucs_b_count_in_5, on=['user_id', 'cate', 'shop_id'],
                                  how='left').fillna(0)
        ...
        # ucs_b_count_in_n.to_csv('../data/features/features_a_test/ucs_b_count_in_n.csv', index=False, encoding='utf-8')
    ucs_features = pd.merge(ucs_features, ucs_b_count_in_n, on=['user_id', 'cate', 'shop_id'], how='left').fillna(0)
    print('ucs_b_count_in_n')

    # (3)ucs_bi_count_in_n(n=3/5/7;i=1/2/3/4/5; 用户在考察日前n天的各类行为总量计数，考察周取2.21-2.27)
    Path = '../data/features/features_a_test/ucs_bi_count_in_n.csv'
    if os.path.exists(Path):
        ucs_bi_count_in_n = pd.read_csv(Path)
    else:
        userSub = userAll[(userAll['action_time'] > end_time_1) & (userAll['action_time'] <= end_time)]
        userSub = userSub[['user_id', 'cate', 'shop_id', 'type_1', 'type_2', 'type_5']]
        userSub = userSub.groupby(['user_id', 'cate', 'shop_id'], as_index=False).sum()
        ...
        userSub['ucs_b5_count_in_1'] = userSub['type_5']  # 用户在考察周前1天的加购（5）行为总量计数
        userSub.drop(['type_1'], axis=1, inplace=True)
        userSub.drop(['type_2'], axis=1, inplace=True)
        userSub.drop(['type_5'], axis=1, inplace=True)
        ...
        ucs_bi_count_in_n = pd.merge(ucs_bi_count_in_7, ucs_bi_count_in_5, on=['user_id',  'cate', 'shop_id'],
                                   how='left').fillna(0)
        ...
        ucs_bi_count_in_n = pd.merge(ucs_bi_count_in_n, ucs_bi_count_in_1, on=['user_id', 'cate', 'shop_id'],
                                     how='left').fillna(0)
        # print(ucs_bi_count_in_n.info())
        # print(ucs_bi_count_in_n.ucs_b2_count_in_7.max())
        # ucs_bi_count_in_n.to_csv('../data/features/features_a_test/ucs_bi_count_in_n.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/ucs_b2_diff_day.csv'
    if os.path.exists(Path):
        ...
        userSub = userSub.groupby(['user_id', 'cate', 'shop_id'])['action_time'].nunique().reset_index(name='ucs_ac_nunique')
        userSub = pd.merge(usertmp, userSub, on=['user_id', 'cate', 'shop_id'], how='left')

        userSub['ucs_b2_diff_day'] = (pd.to_datetime(userSub['action_time_2']) - pd.to_datetime(userSub['action_time_1'])).dt.days
        # print(usertmp.head(20))
        ...
        # ucs_b2_diff_day.to_csv('../data/features/features_a_test/ucs_b2_diff_day.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/ucs_b1_last_day.csv'
    if os.path.exists(Path):
        ...
        usertmp['ucs_b1_last_min_day'] = (pd.to_datetime(label_time_start) - pd.to_datetime(usertmp['action_time'])).dt.days
        usertmp['ucs_b1_last_max_day'] = (pd.to_datetime(label_time_end) - pd.to_datetime(usertmp['action_time'])).dt.days
        # print(usertmp.head(20))
        usertmp = usertmp[['user_id', 'cate', 'shop_id', 'ucs_b1_last_min_day', 'ucs_b1_last_max_day']]
        # usertmp.to_csv('../data/features/features_a_test/ucs_b1_last_day.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/ucs_active_days.csv'
    if os.path.exists(Path):
        ...
        # ucs_active_days.to_csv('../data/features/features_a_test/ucs_active_days.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/ucs_b_count_rank_in_n_in_u.csv'
    if os.path.exists(Path):
        ...
        userSub = userSub[['user_id', 'cate', 'shop_id', 'type_1', 'type_2', 'type_3', 'type_4', 'type_5']]
        userSub = userSub.groupby(['user_id', 'cate', 'shop_id'], as_index=False).sum()
        userSub['b_count'] = userSub['type_1'] + userSub['type_2'] + userSub['type_3'] + userSub['type_4'] + userSub[
            'type_5']
        userSub.drop(['type_1', 'type_2', 'type_3', 'type_4', 'type_5'], axis=1, inplace=True)
        ...
        # ucs_b_count_rank_in_n_in_u.to_csv('../data/features/features_a_test/ucs_b_count_rank_in_n_in_u.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/ucs_buy_info.csv'
    if os.path.exists(Path):
        ...
        userSub = userSub[(userSub.type_2 == 1)]
        usertmp = userSub.groupby(['user_id', 'shop_id', 'cate'])['action_time'].nunique().reset_index(name='ucs_ac_nunique')
        ...
        usertmp['ucs_b2_last_day_end'] = (pd.to_datetime(label_time_end) - pd.to_datetime(usertmp['action_time'])).dt.days

        userSub = userSub.merge(usertmp, on=['user_id', 'shop_id', 'cate'], how='left')
        userSub.drop(['type_2', 'action_time'], axis=1, inplace=True)
        # userSub.to_csv('../data/features/features_a_test/ucs_buy_info.csv', index=False, encoding='utf-8')
    ...
    filePath = '../data/features/features_a_test/ucs_features.csv'
...
def get_cateInfo_features():
    ...
    Path = '../data/features/features_a_test/c_b2_rate.csv'
    if os.path.exists(Path):
        ...
        # c_b2_rate.to_csv('../data/features/features_a_test/c_b2_rate.csv', index=False, encoding='utf-8')
    cate_features = pd.merge(cate_features, c_b2_rate, on='cate', how='left').fillna(0)
    print('c_b2_rate')

    # (2)c_all_count(品类下商品，品牌，店铺的数量)
    Path = '../data/features/features_a_test/c_all_count.csv'
    if os.path.exists(Path):
        ...
        print(userSub.head())
        userSub.rename(columns={'sku_id': 'c_item_count', 'brand': 'c_brand_count', 'shop_id': 'c_shop_count'}, inplace=True)
        c_all_count = userSub.copy()
        # c_all_count.to_csv('../data/features/features_a_test/c_all_count.csv', index=False, encoding='utf-8')
    cate_features = pd.merge(cate_features, c_all_count, on='cate', how='left').fillna(0)
    print('c_all_count')

    # (3)c_new_item_count(品类下新品的数量)
    Path = '../data/features/features_a_test/c_new_item_count.csv'
    if os.path.exists(Path):
        ...
        # c_new_item_count.to_csv('../data/features/features_a_test/c_new_item_count.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/c_u_count_in_n.csv'
    if os.path.exists(Path):
        ...
        # c_u_count_in_n.to_csv('../data/features/features_a_test/c_u_count_in_n.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/c_bi_count_in_n.csv'
    if os.path.exists(Path):
        c_bi_count_in_n = pd.read_csv(Path)
    else:
        userSub = userAll[(userAll['action_time'] > end_time_1) & (userAll['action_time'] <= end_time)]
        userSub = userSub.drop_duplicates()
        userSub = userSub[['cate', 'type_5']]
        userSub = userSub.groupby(['cate'], as_index=False).sum()
        # print(userSub.head())
        userSub.rename(columns={'type_5': 'c_b5_count_in_1'}, inplace=True)
        # print(userSub.info())
        # print(userSub.head())
        ...
        # c_bi_count_in_n.to_csv('../data/features/features_a_test/c_bi_count_in_n.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/c_b_count_in_n.csv'
    if os.path.exists(Path):
        c_b_count_in_n = pd.read_csv(Path)
    else:
        userSub = c_bi_count_in_n.copy()
        userSub['c_b_count_in_3'] = userSub['c_b1_count_in_3'] + userSub['c_b2_count_in_3'] + userSub['c_b3_count_in_3'] + \
                                    userSub['c_b4_count_in_3']
        userSub['c_b_count_in_5'] = userSub['c_b1_count_in_5'] + userSub['c_b2_count_in_5'] + userSub['c_b3_count_in_5'] + \
                                    userSub['c_b4_count_in_5']
        userSub['c_b_count_in_7'] = userSub['c_b1_count_in_7'] + userSub['c_b2_count_in_7'] + userSub['c_b3_count_in_7'] + \
                                    userSub['c_b4_count_in_7']
        userSub = userSub[['cate', 'c_b_count_in_3', 'c_b_count_in_5', 'c_b_count_in_7']]
        # print(userSub.info())
        # print(userSub.head())
        c_b_count_in_n = userSub.copy()
        # c_b_count_in_n.to_csv('../data/features/features_a_test/c_b_count_in_n.csv', index=False, encoding='utf-8')
    cate_features = pd.merge(cate_features, c_b_count_in_n, on='cate', how='left').fillna(0)
    print('c_b_count_in_n')

    # (7)c_b2_count(类别被购买的频次)
    Path = '../data/features/features_a_test/c_b2_count.csv'
    if os.path.exists(Path):
        ...
        # c_b2_count.to_csv('../data/features/features_a_test/c_b2_count.csv', index=False, encoding='utf-8')
    cate_features = pd.merge(cate_features, c_b2_count, on='cate', how='left').fillna(0)
    print('c_b2_count')

    ## (8)品类的评论
    Path = '../data/features/features_a_test/c_comments_counts.csv'
    if os.path.exists(Path):
        ...
        userSub = userSub.groupby(['cate'], as_index=False).sum()
        userSub.rename(columns={'comments':'c_comments', 'good_comments':'c_good_comments', 'bad_comments':'c_bad_comments'}, inplace=True)
        # userSub.to_csv('../data/features/features_a_test/c_comments_counts.csv', index=False, encoding='utf-8')
    ...
    filePath = '../data/features/features_a_test/cate_features.csv'
...
def get_shopInfo_features():
    ...
    Path = '../data/features/features_a_test/s_b2_rate.csv'
    if os.path.exists(Path):
        ...
        # s_b2_rate.to_csv('../data/features/features_a_test/s_b2_rate.csv', index=False, encoding='utf-8')
    shop_features = pd.merge(shop_features, s_b2_rate, on='shop_id', how='left').fillna(0)
    print('s_b2_rate')

    # (2)s_all_count(店铺下品类,商品的个数)
    Path = '../data/features/features_a_test/s_all_count.csv'
    if os.path.exists(Path):
        ...
        # s_all_count.to_csv('../data/features/features_a_test/s_all_count.csv', index=False, encoding='utf-8')
    shop_features = pd.merge(shop_features, s_all_count, on='shop_id', how='left').fillna(0)
    print('s_all_count')

    # (3)s_new_item_count(店铺下新品的数量)
    Path = '../data/features/features_a_test/s_new_item_count.csv'
    if os.path.exists(Path):
        ...
        # s_new_item_count.to_csv('../data/features/features_a_test/s_new_item_count.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/s_u_count_in_n.csv'
    if os.path.exists(Path):
        ...
        # s_u_count_in_n.to_csv('../data/features/features_a_test/s_u_count_in_n.csv', index=False, encoding='utf-8')
    shop_features = pd.merge(shop_features, s_u_count_in_n, on='shop_id', how='left').fillna(0)

    # (3)c_bi_count_in_n(店铺在考察日前n天的各项行为计数  反映了店铺的热度（用户操作吸引），包含着店铺产生的购买习惯特点)
    Path = '../data/features/features_a_test/s_bi_count_in_n.csv'
    if os.path.exists(Path):
        s_bi_count_in_n = pd.read_csv(Path)
    else:
        userSub = userAll[(userAll['action_time'] > end_time_1) & (userAll['action_time'] <= end_time)]
        userSub = userSub.drop_duplicates()
        userSub = userSub[['shop_id', 'type_5']]
        userSub = userSub.groupby(['shop_id'], as_index=False).sum()
        # print(userSub.head())
        userSub.rename(columns={'type_5': 's_b5_count_in_1'}, inplace=True)
        # print(userSub.info())
        # print(userSub.head())
        ...
        # s_bi_count_in_n.to_csv('../data/features/features_a_test/s_bi_count_in_n.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/s_b_count_in_n.csv'
    if os.path.exists(Path):
        ...
        # s_b_count_in_n.to_csv('../data/features/features_a_test/s_b_count_in_n.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/s_b2_diff_day.csv'
    if os.path.exists(Path):
        ...
        # s_b2_diff_day.to_csv('../data/features/features_a_test/s_b2_diff_day.csv', index=False, encoding='utf-8')
    shop_features = pd.merge(shop_features, s_b2_diff_day, on='shop_id', how='left').fillna(0)

    ## (6)店铺评论
    Path = '../data/features/features_a_test/s_comments_counts.csv'
    if os.path.exists(Path):
        ...
        userSub.rename(
            ...
            inplace=True)
        # userSub.to_csv('../data/features/features_a_test/s_comments_counts.csv', index=False, encoding='utf-8')
    ...
    ## (7)店铺信息
    Path = '../data/features/features_a_test/shop_info.csv'
    if os.path.exists(Path):
        ...
        # shop_info.to_csv('../data/features/features_a_test/shop_info.csv', index=False, encoding='utf-8')
    shop_features = pd.merge(shop_features, shop_info, on='shop_id', how='left').fillna(0)

    filePath = '../data/features/features_a_test/shop_features.csv'
...
def get_user_cate_features():
    ...
    Path = '../data/features/features_a_test/uc_bi_last_day.csv'
    if os.path.exists(Path):
        uc_bi_last_day = pd.read_csv(Path)
    else:
        userSub = userAll[(userAll['action_time'] >= start_time) & (userAll['action_time'] <= end_time)]
        userSub = userSub[['user_id', 'cate', 'type_1', 'type_2', 'type_3', 'type_4', 'type_5', 'action_time']]
        ...
        userSub5 = userSub[(userSub.type_5 == 1)][['user_id', 'cate', 'type_5', 'action_time']]
        ...
        userSub1['uc_b1_last_day'] = (pd.to_datetime(label_time_start) - pd.to_datetime(userSub1['action_time'])).dt.days
        ...
        userSub2['uc_b2_last_day'] = (pd.to_datetime(label_time_start) - pd.to_datetime(userSub2['action_time'])).dt.days
        userSub2.drop(['action_time', 'type_2'], axis=1, inplace=True)
        # print(userSub2.head(20))

        userSub5 = userSub5.sort_values(['user_id', 'cate', 'action_time'])
        # print(userSub5.head(20))
        userSub5 = userSub5.drop_duplicates(['user_id', 'cate'], keep='last')
        userSub5['uc_b5_last_day'] = (pd.to_datetime(label_time_start) - pd.to_datetime(userSub5['action_time'])).dt.days
        userSub5.drop(['action_time', 'type_5'], axis=1, inplace=True)
        # print(userSub5.head(20))

        uc_bi_last_day = pd.merge(userSub1, userSub2, how='left', on=['user_id', 'cate']).fillna(60)
        uc_bi_last_day = pd.merge(uc_bi_last_day, userSub5, how='left', on=['user_id', 'cate']).fillna(60)
        # print(uc_bi_last_hours.info())
        # print(uc_bi_last_hours.head())
        # uc_bi_last_day.to_csv('../data/features/features_a_test/uc_bi_last_day.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/uc_b2_diff_day.csv'
    if os.path.exists(Path):
        ...
        userSub['uc_b2_diff_day'] = (pd.to_datetime(userSub['action_time_2']) - pd.to_datetime(userSub['action_time_1'])).dt.days
        ...
        # uc_b2_diff_day.to_csv('../data/features/features_a_test/uc_b2_diff_day.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/uc_b_count_in_n.csv'
    if os.path.exists(Path):
        ...
        uc_b_count_in_n = pd.merge(uc_b_count_in_7, uc_b_count_in_5, how='left').fillna(0)
        ...
        # uc_b_count_in_n.to_csv('../data/features/features_a_test/uc_b_count_in_n.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/uc_bi_count_in_n.csv'
    if os.path.exists(Path):
        uc_bi_count_in_n = pd.read_csv(Path)
    else:
        userSub = userAll[(userAll['action_time'] > end_time_1) & (userAll['action_time'] <= end_time)]
        userSub = userSub[['user_id', 'cate', 'type_5']]
        userSub.rename(columns={'type_5': 'uc_b5_count_in_1'}, inplace=True)
        userSub = userSub.groupby(['user_id', 'cate'], as_index=False).sum()
        # print(userSub.info())
        # print(userSub.head())
        ...
        uc_bi_count_in_n = pd.merge(uc_bi_count_in_7, uc_bi_count_in_5, how='left').fillna(0)
        ...
        # uc_bi_count_in_n.to_csv('../data/features/features_a_test/uc_bi_count_in_n.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/uc_b_count_rank_in_n_in_u.csv'
    if os.path.exists(Path):
        uc_b_count_rank_in_n_in_u = pd.read_csv(Path)
    else:
        userSub = userAll[(userAll['action_time'] > end_time_1) & (userAll['action_time'] <= end_time)]
        userSub = userSub[['user_id', 'cate', 'type_1', 'type_2', 'type_3', 'type_4', 'type_5']]
        userSub = userSub.groupby(['user_id', 'cate'], as_index=False).sum()
        userSub['b_count'] = userSub['type_1'] + userSub['type_2'] + userSub['type_3'] + userSub['type_4'] + userSub[
            'type_5']
        userSub.drop(['type_1', 'type_2', 'type_3', 'type_4', 'type_5'], axis=1, inplace=True)
        ...
        uc_b_count_rank_in_n_in_u = pd.merge(uc_b_count_rank_in_n_in_u, uc_b_count_rank_in_1_in_u, how='left',
                                             on=['user_id', 'cate']).fillna(1000)
        # uc_b_count_rank_in_n_in_u.to_csv('../data/features/features_a_test/uc_b_count_rank_in_n_in_u.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/uc_b2_rate.csv'
    if os.path.exists(Path):
        ...
        # uc_b2_rate.to_csv('../data/features/features_a_test/uc_b2_rate.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/uc_active_days.csv'
    if os.path.exists(Path):
        ...
        # uc_active_days.to_csv('../data/features/features_a_test/uc_active_days.csv', index=False, encoding='utf-8')
    user_cate_features = pd.merge(user_cate_features, uc_active_days, on=['user_id', 'cate'], how='left').fillna(0)
    print('uc_active_days')
    filePath = '../data/features/features_a_test/user_cate_features.csv'
...
def  get_user_shop_features():
    ...
    ###    -  us_bi_count_in_n(用户-类别对在考察日前n天的各项行为计数 反映了user_id -item_category的活跃度，
    ###    -  反映了user_id -item_category的各项操作的活跃度，对应着user_id -item_category的购买习惯)
    ...
    Path = '../data/features/features_a_test/us_bi_last_day.csv'
    if os.path.exists(Path):
        us_bi_last_day = pd.read_csv(Path)
    else:
        userSub = userAll[(userAll['action_time'] >= start_time) & (userAll['action_time'] <= end_time)]
        userSub = userSub[['user_id', 'shop_id', 'type_1', 'type_2', 'type_5', 'action_time']]
        ...
        userSub5 = userSub[(userSub.type_5 == 1)][['user_id', 'shop_id', 'type_5', 'action_time']]

        userSub1 = userSub1.sort_values(['user_id', 'shop_id', 'action_time'])
        # print(userSub1.head(20))
        userSub1 = userSub1.drop_duplicates(['user_id', 'shop_id'], keep='last')
        userSub1['us_b1_last_day'] = (pd.to_datetime(label_time_start) - pd.to_datetime(userSub1['action_time'])).dt.days
        userSub1.drop(['action_time', 'type_1'], axis=1, inplace=True)
        # print(userSub1.head(20))
        ...
        userSub2['us_b2_last_day'] = (pd.to_datetime(label_time_start) - pd.to_datetime(userSub2['action_time'])).dt.days
        userSub2.drop(['action_time', 'type_2'], axis=1, inplace=True)
        # print(userSub2.head(20))

        userSub5 = userSub5.sort_values(['user_id', 'shop_id', 'action_time'])
        # print(userSub5.head(20))
        userSub5 = userSub5.drop_duplicates(['user_id', 'shop_id'], keep='last')
        userSub5['us_b5_last_day'] = (pd.to_datetime(label_time_start) - pd.to_datetime(userSub5['action_time'])).dt.days
        userSub5.drop(['action_time', 'type_5'], axis=1, inplace=True)
        # print(userSub5.head(20))

        us_bi_last_day = pd.merge(userSub1, userSub2, how='left', on=['user_id', 'shop_id']).fillna(60)
        us_bi_last_day = pd.merge(us_bi_last_day, userSub5, how='left', on=['user_id', 'shop_id']).fillna(60)
        # print(us_bi_last_day.info())
        # print(us_bi_last_day.head())
        # us_bi_last_day.to_csv('../data/features/features_a_test/us_bi_last_day.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/us_b2_diff_day.csv'
    if os.path.exists(Path):
        ...
        # us_b2_diff_day.to_csv('../data/features/features_a_test/us_b2_diff_day.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/us_b_count_in_n.csv'
    if os.path.exists(Path):
        ...
        us_b_count_in_n = pd.merge(us_b_count_in_7, us_b_count_in_5, how='left').fillna(0)
        ...
        # us_b_count_in_n.to_csv('../data/features/features_a_test/us_b_count_in_n.csv', index=False, encoding='utf-8')
    ...
    # 反映了user_id -item_category的各项操作的活跃度，对应着user_id -item_category的购买习惯)
    Path = '../data/features/features_a_test/us_bi_count_in_n.csv'
    if os.path.exists(Path):
        us_bi_count_in_n = pd.read_csv(Path)
    else:
        userSub = userAll[(userAll['action_time'] > end_time_1) & (userAll['action_time'] <= end_time)]
        userSub = userSub[['user_id', 'shop_id', 'type_1', 'type_2', 'type_3', 'type_4', 'type_5']]
        userSub.rename(
            columns={'type_1': 'us_b1_count_in_1', 'type_2': 'us_b2_count_in_1', 'type_3': 'us_b3_count_in_1',
                     'type_4': 'us_b4_count_in_1', 'type_5': 'us_b5_count_in_1'}, inplace=True)
        userSub = userSub.groupby(['user_id', 'shop_id'], as_index=False).sum()
        # print(userSub.info())
        # print(userSub.head())
        ...
        userSub = userAll[(userAll['action_time'] >= end_time_3) & (userAll['action_time'] <= end_time)]
        userSub = userSub[['user_id', 'shop_id', 'type_1', 'type_2', 'type_3', 'type_4']]
        userSub.rename(columns={'type_1':'us_b1_count_in_3', 'type_2':'us_b2_count_in_3', 'type_3':'us_b3_count_in_3', 'type_4':'us_b4_count_in_3'}, inplace=True)
        userSub = userSub.groupby(['user_id', 'shop_id'], as_index=False).sum()
        # print(userSub.info())
        # print(userSub.head())
        ...
        userSub = userAll[(userAll['action_time'] >= end_time_5) & (userAll['action_time'] <= end_time)]
        userSub = userSub[['user_id', 'shop_id', 'type_1', 'type_2', 'type_3', 'type_4']]
        userSub.rename(columns={'type_1':'us_b1_count_in_5', 'type_2':'us_b2_count_in_5', 'type_3':'us_b3_count_in_5', 'type_4':'us_b4_count_in_5'}, inplace=True)
        userSub = userSub.groupby(['user_id', 'shop_id'], as_index=False).sum()
        # print(userSub.info())
        # print(userSub.head())
        ...
        userSub = userAll[(userAll['action_time'] >= end_time_7) & (userAll['action_time'] <= end_time)]
        userSub = userSub[['user_id', 'shop_id', 'type_1', 'type_2', 'type_3', 'type_4']]
        userSub.rename(columns={'type_1':'us_b1_count_in_7', 'type_2':'us_b2_count_in_7', 'type_3':'us_b3_count_in_7', 'type_4':'us_b4_count_in_7'}, inplace=True)
        userSub = userSub.groupby(['user_id', 'shop_id'], as_index=False).sum()
        # print(userSub.info())
        # print(userSub.head())
        us_bi_count_in_7 = userSub.copy()

        us_bi_count_in_n = pd.merge(us_bi_count_in_7, us_bi_count_in_5, how='left').fillna(0)
        ...
        # us_bi_count_in_n.to_csv('../data/features/features_a_test/us_bi_count_in_n.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/us_b_count_rank_in_n_in_u.csv'
    if os.path.exists(Path):
        ...
        userSub['b_count'] = userSub['type_1'] + userSub['type_2'] + userSub['type_3'] + userSub['type_4']
        userSub.drop(['type_1', 'type_2', 'type_3', 'type_4'], axis=1, inplace=True)
        userSub['us_b_count_rank_in_3_in_u'] = userSub['b_count'].groupby(userSub['user_id']).rank(ascending=0, method='dense')
        userSub.drop('b_count', axis=1, inplace=True)
        ...
        userSub['b_count'] = userSub['type_1'] + userSub['type_2'] + userSub['type_3'] + userSub['type_4']
        userSub.drop(['type_1', 'type_2', 'type_3', 'type_4'], axis=1, inplace=True)
        userSub['us_b_count_rank_in_5_in_u'] = userSub['b_count'].groupby(userSub['user_id']).rank(ascending=0, method='dense')
        userSub.drop('b_count', axis=1, inplace=True)
        ...
        userSub['b_count'] = userSub['type_1'] + userSub['type_2'] + userSub['type_3'] + userSub['type_4']
        userSub.drop(['type_1', 'type_2', 'type_3', 'type_4'], axis=1, inplace=True)
        userSub['us_b_count_rank_in_7_in_u'] = userSub['b_count'].groupby(userSub['user_id']).rank(ascending=0, method='dense')
        userSub.drop('b_count', axis=1, inplace=True)
        ...
        us_b_count_rank_in_n_in_u = pd.merge(us_b_count_rank_in_n_in_u, us_b_count_rank_in_1_in_u, how='left',
                                             on=['user_id', 'shop_id']).fillna(1000)
        # us_b_count_rank_in_n_in_u.to_csv('../data/features/features_a_test/us_b_count_rank_in_n_in_u.csv', index=False, encoding='utf-8')
        # print(us_b_count_rank_in_n_in_u.info())
        # print(us_b_count_rank_in_n_in_u.head())
    user_shop_features = pd.merge(user_shop_features, us_b_count_rank_in_n_in_u, on=['user_id', 'shop_id'], how='left').fillna(0)

    # 6、us_b2_rate(用户-类别的点击购买转化率  反映了item_category的购买决策操作特点)
    Path = '../data/features/features_a_test/us_b2_rate.csv'
    if os.path.exists(Path):
        ...
        # print(c_b2_rate.info())
        # print(c_b2_rate.head())
        # us_b2_rate.to_csv('../data/features/features_a_test/us_b2_rate.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/us_active_days.csv'
    if os.path.exists(Path):
        ...
        # us_active_days.to_csv('../data/features/features_a_test/us_active_days.csv', index=False, encoding='utf-8')
    user_shop_features = pd.merge(user_shop_features, us_active_days, on=['user_id', 'shop_id'], how='left').fillna(0)


    filePath = '../data/features/features_a_test/user_shop_features.csv'
...
def get_cate_shop_features():
    ...
    Path = '../data/features/features_a_test/cs_u_rank_in_s.csv'
    if os.path.exists(Path):
        ...
        # cs_u_rank_in_s.to_csv('../data/features/features_a_test/cs_u_rank_in_s.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/cs_item_count.csv'
    if os.path.exists(Path):
        ...
        # cs_item_count.to_csv('../data/features/features_a_test/cs_item_count.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/cs_new_item_count.csv'
    if os.path.exists(Path):
        ...
        # cs_new_item_count.to_csv('../data/features/features_a_test/cs_new_item_count.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/cs_b2_count.csv'
    if os.path.exists(Path):
        ...
        # cs_b2_count.to_csv('../data/features/features_a_test/cs_b2_count.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/cs_b_rank_in_s.csv'
    if os.path.exists(Path):
        cs_b_rank_in_s = pd.read_csv(Path)
    else:
        userSub = userAll[(userAll['action_time'] >= start_time) & (userAll['action_time'] <= end_time)]
        userSub = userSub[['cate', 'shop_id', 'type_1', 'type_2', 'type_3', 'type_4', 'type_5']]
        # print(userSub.info())
        # print(userSub.head())
        userSub.insert(6, 'b_count',
                       value=userAll['type_1'] + userAll['type_2'] + userAll['type_3'] + userAll['type_4'] + userAll[
                           'type_5'])
        # print(userSub.info())
        # print(userSub.head())
        userSub = userSub.drop(['type_1', 'type_2', 'type_3', 'type_4', 'type_5'], axis=1)
        # print(userSub.head())
        userSub = userSub.groupby(['cate', 'shop_id'], as_index=False).sum()
        ...
        # print(userSub.head(30))
        userSub.drop(['b_count'], axis=1, inplace=True)
        # userSub.to_csv('../data/features/features_a_test/cs_b_rank_in_s.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/cs_b2_rank_in_s.csv'
    if os.path.exists(Path):
        ...
        userSub.drop('type_2', axis=1, inplace=True)
        # userSub.to_csv('../data/features/features_a_test/cs_b2_rank_in_s.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/cs_b2_rate.csv'
    if os.path.exists(Path):
        ...
        # cs_b2_rate.to_csv('../data/features/features_a_test/cs_b2_rate.csv', index=False, encoding='utf-8')
    ...
    Path = '../data/features/features_a_test/cs_active_days.csv'
    if os.path.exists(Path):
        ...
        # cs_active_days.to_csv('../data/features/features_a_test/cs_active_days.csv', index=False, encoding='utf-8')
    cate_shop_features = pd.merge(cate_shop_features, cs_active_days, on=['cate', 'shop_id'], how='left').fillna(0)

    # (6)是否是主营品类
    Path = '../data/features/features_a_test/cs_major_cate.csv'
    if os.path.exists(Path):
        ...
        # cs_major_cate.to_csv('../data/features/features_a_test/cs_major_cate.csv', index=False, encoding='utf-8')
    cate_shop_features = pd.merge(cate_shop_features, cs_major_cate, on=['cate', 'shop_id'], how='left').fillna(0)

    filePath = '../data/features/features_a_test/cate_shop_features.csv'
